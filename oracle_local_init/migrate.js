...
async function migrateJurinet(action = 'create') {
    ...
    const queryString = await readFile(resolve(__dirname, `jurinet_${action}_schema.sql`), 'utf8')
...
async function migrateJurica(action = 'create') {
    ...
    const queryString = await readFile(resolve(__dirname, `jurica_${action}_schema.sql`), 'utf8')
    const queries = splitQueries(queryString)
...
async function migratePenal(action = 'create') {
    ...
    const queryString = await readFile(resolve(__dirname, `penal_${action}_schema.sql`), 'utf8')
    const queries = splitQueries(queryString)
...
async function migrateGrcom(action = 'create') {
    ...
    const queryString = await readFile(resolve(__dirname, `grcom_${action}_schema.sql`), 'utf8')
    const queries = splitQueries(queryString)
...
async function main() {
    try {
        const command = 
            process.argv[2] === 'up' ? 'create' :
            process.argv[2] === 'down' ? 'drop' :
            null
