...
class PenalOracle {
  ...
  async connect() {
    if (this.connected === false) {
      this.connection = await oracledb.getConnection({
        user: process.env.PENAL_DB_USER,
        password: process.env.PENAL_DB_PASS,
        connectString: process.env.PENAL_DB_HOST,
      });
      this.connected = true;
    } else {
      throw new Error('PenalOracle.connect: already connected.');
    }
  }
  ...
  async close() {
    if (this.connected === true && this.connection !== null) {
      ...
      throw new Error('PenalOracle.close: not connected.');
    }
  }
  ...
  async describe(table) {
    if (this.connected === true && this.connection !== null) {
      ...
      throw new Error('PenalOracle.describe: not connected.');
    }
  }
  ...
  async buildRawData(row) {
    if (this.connected === true && this.connection !== null) {
      ...
      throw new Error('PenalOracle.buildRawData: not connected.');
    }
  }
}

exports.PenalOracle = PenalOracle;
