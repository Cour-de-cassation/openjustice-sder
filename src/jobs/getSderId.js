  ...
  for await (const doc of cursor) {
    try {
      ...
      if (update.modifiedCount > 0)
        CustomLog.log('info', {
          operationName: 'batch getSderId',
          msg: `sderId: ${doc._id} added to ${doc.sourceName}:${doc.sourceId}`,
        });
      else
        CustomLog.log('info', {
          operationName: 'batch getSderId',
          msg: `sderId: ${doc._id} already known to ${doc.sourceName}:${doc.sourceId}`,
        });
    } catch (err) {
      CustomLog.log('error', {
        operationName: 'batch getSderId',
        msg: `error while adding sderId: ${doc._id} to ${doc.sourceName}:${doc.sourceId}`,
        data: err.stack,
      });
    }
  }
