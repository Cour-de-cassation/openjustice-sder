...
async function main() {
  ...
  if (jurinetResult) {
      if (row['AUT_CREATION'] !== 'WINCI' && row['TYPE_ARRET'] === 'CC') {
        try {
          ...
          if (normalized === null) {
            ...
            if (normDec.pseudoText) {
              try {
                ...
                if (!normDec.zoning || (normDec.zoning && normDec.zoning.detail)) {
                  fs.appendFileSync(
                    path.join(__dirname, 'zoningerror_jurinet.log'),
                    JSON.stringify({ id: normDec.sourceId, error: parseError(normDec.zoning) }) + '\n',
                  );
                ...
                fs.appendFileSync(
                  path.join(__dirname, 'zoningerror_jurinet.log'),
                  JSON.stringify({ id: normDec.sourceId, error: parseError(e) }) + '\n',
                );
            ...
            if (
              updated === true ||
              normalized._version !== decisionsVersion ||
              !normalized.zoning ||
              (normalized.zoning && normalized.zoning.detail)
            ) {
    ...
    console.log(
      `Jurinet (${oldOffset}-${
        oldOffset + batchSize
      }) New: ${newCount}, Update: ${updateCount}, Normalize: ${normalizeCount}, Error: ${errorCount}.`,
    );
        ...
        if (normalized === null) {
          ...
          if (normDec.pseudoText) {
            try {
              ...
              if (!normDec.zoning || (normDec.zoning && normDec.zoning.detail)) {
                fs.appendFileSync(
                  path.join(__dirname, 'zoningerror_jurica.log'),
                  JSON.stringify({ id: normDec.sourceId, error: parseError(normDec.zoning) }) + '\n',
                );
                console.error(normDec.sourceName, normDec.sourceId, 'zoning error', parseError(normDec.zoning));
              }
...
async function mainShort() {
  ...
  if (jurinetResult) {
    ...
    console.log(
      `Jurinet (latest 5000) New: ${newCount}, Update: ${updateCount}, Normalize: ${normalizeCount}, Error: ${errorCount}.`,
    );
    await client.close();
  }
  ...
  if (juricaResult) {
    ...
    console.log(
      `Jurica (latest 5000) New: ${newCount}, Update: ${updateCount}, Normalize: ${normalizeCount}, Error: ${errorCount}.`,
    );
    await client.close();
  }
