...
async function importJurinet() {
  ...
  if (jurinetResult) {
    for (let i = 0; i < jurinetResult.length; i++) {
      ...
      if (
        row['TYPE_ARRET'] === 'CC' ||
        (row['TYPE_ARRET'] === 'AUTRE' && /^t\.cfl$/i.test(row['ID_CHAMBRE']) === true)
      ) {
        try {
  ...
  CustomLog.log('info', {
    ...
    msg: `Done Importing Jurinet - New: ${newCount}, Error: ${errorCount}, Skip: ${skipCount}.`,
  });
  return true;
}
...
async function importJurica() {
  ...
  CustomLog.log('info', {
    ...
    msg: `Done Importing Jurica - New: ${newCount}, Non-public: ${nonPublicCount}, Error: ${errorCount}, Skip: ${skipCount}.`,
  });
  return true;
}
